class Renderer {
    ...
    drawCharacter(character, isFlashing = false, opponentX = null) {
        if (!character) return;

        const imageUrl = character.getCurrentImage(opponentX);
        if (!imageUrl) return;
        ...
        if (img && img.complete && img.naturalWidth > 0) {
            ...
            // Draw character at their position (no flipping needed, image direction is handled by getCurrentImage)
